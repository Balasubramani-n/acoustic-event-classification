...
class UrbanSoundDataset(Dataset) :    
    ...
    def __init__(self,spec , toDB , train = True, test_fold = [1,2]  , kfold = 10) :
    ...
    def __getitem__(self,index):
        ...
        if self.spec == "mfcc":
            signal = self.mfcc_t(signal)
            if self.toDB :
                signal = self.amp_to_DB(mfcc)
        elif self.spec == "lfcc":
            signal = self.lfcc_t(signal)
            if self.toDB :
                signal = self.amp_to_DB(lfcc)
        elif self.spec == "mel" :
            signal = self.mel_t(signal)
            if self.toDB :
                signal = self.amp_to_DB(mel)
        else :
            raise ValueError("No match for specified transformation!!!")
